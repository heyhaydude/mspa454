...
```{r}
forest.t = forest.train

#one area variable
idxs = grep("Elevation|LDist|shade.pca|Roadway|FirePoint|CoverType",colnames(forest.train))
forest.t = data.frame(forest.train[,idxs])

mat = as.matrix(forest.train[,grep("Area",colnames(forest.train))])
forest.t$Area = factor(mat%*%(1:ncol(mat)), labels = colnames(mat)) 
forest.t$CoverType = as.factor(forest.t$CoverType)

# percentage by class before sampling
forest.t.proptable = round(prop.table(table(forest.t1$CoverType)),digits = 3)

m = SMOTE(CoverType ~ ., forest.t1, perc.over = 100, perc.under=100)

r2 = round(prop.table(table(m$CoverType)),digits = 3)

rbind(r1,r2)

# train set 1: downsample all classes to lowest observation class count. 
# 1,917 each, 13,300 total
forest.t1 = as.data.frame(forest.t %>% group_by(CoverType) %>% sample_n(size = 1900))

# train set 2: get 10,000 observations in all classes with up and down sampling
forest.t1 
match1 = forest.t$CoverType==4
match2 = forest.t$CoverType==1
matches = match1|match2
sum1 = sum(match1)
sum2 = sum(match2)
percover = sum1/sum2 * 100
temp = forest.t[matches,]
temp$CoverType = as.numeric(temp$CoverType)
temp$CoverType = as.factor(temp$CoverType)
m = SMOTE(CoverType ~ ., temp, perc.under = 100)
table(m$CoverType)
