...
### clear out some of the memory not needed anymore
forest = NULL
forest.numeric = NULL
forest.pca = NULL
forest.scaled = NULL
forest.transforms = NULL
soil.pca.x = NULL
shadeslope.pca.x = NULL
forest.bucketing = NULL
...
idxs = grep("^SoilType|^Aspect|^Hillshade|^Slope|^HDist.Hydrology|VDist.Hydrology",colnames(forest.model))
forest.model[,idxs] = NULL
...
sample1.cm = confusionMatrix(sample1.pred,forest.valid$CoverType) # kappa 0.5283
...
sample2.cm = confusionMatrix(sample2.pred, forest.valid$CoverType) # kappa 0.6378
...
sample3.cm = confusionMatrix(sample3.pred, forest.valid$CoverType) # kappa 
...
sample4.cm = confusionMatrix(sample4.pred, forest.valid$CoverType) # kappa 0.635
...
sample5.cm = confusionMatrix(sample5.pred, forest.valid$CoverType) # kappa 0.6646
