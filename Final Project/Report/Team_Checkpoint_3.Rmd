...
#model with soil info and assured low class observations, 5 nodes, 0.01 decay
set.seed(444)
forest.model.nn4 = forest.train4
idxs = grep("Elevation|HDist|trans|shade",colnames(forest.model.nn4))
forest.model.nn4[,idxs] = scale(forest.model.nn4[,idxs], center = TRUE, scale = TRUE)
if (build) {
  forest.nn4 = nnet(CoverType ~ . , data=forest.model.nn4, linout=FALSE, size=5, maxit=2000, decay=0.01, trace=FALSE)
...
idxs = grep("Elevation|HDist|trans|shade",colnames(forest.model.nn4.valid))
forest.model.nn4.valid[,idxs] = scale(forest.model.nn4.valid[,idxs], center = TRUE, scale = TRUE)
forest.nn4.pred = predict(forest.nn4, newdata=forest.model.nn4.valid, type="class")
...
m = dummyVars(" ~ .", data = forest.nn5.model[,-idxs])
m = predict(m, newdata = forest.nn5.model)
...
if (build) {
  forest.nn5 = neuralnet(fnn, data=m, hidden=c(9,3), linear.output=FALSE, learningrate = 0.01)
  s = forest.nn5
...
#Modeling
pred.lda <-lda(CoverType ~ . ,data=forest.model.lda1)
...
fitGraph <- ggplotLDAPrep(wine.lda)
