...
idxs = grep("^Aspect|^Hillshade|^Slope|^HDist.Hydrology|VDist.Hydrology|soil.pca|zone.pca",colnames(forest.model))
...
(Write up how we chose which sampling to use)
...
set.seed(444)
idxs = grep("Elevation|HDist.Roadway|HDist.FirePoint|CoverType|trans.LDist.Hydrology|shade.pca|forest.area|forest.zone",colnames(forest.train))
forest.model.nn1 = sample_n(forest.train[,idxs], 40000)
forest.nn1 = nnet(CoverType ~ . , data=forest.model[,idxs], linout=FALSE, size=5, maxit=2000, decay=0.01, trace=FALSE)
...
idxs = grep("Elevation|HDist.Roadway|HDist.FirePoint|CoverType|trans.LDist.Hydrology|shade.pca|forest.area|forest.zone",colnames(forest.train))
sample(forest.train)
n = names(forest.train[,idxs])
f = as.formula(paste("CoverType ~", paste(n[!n %in% "CoverType"], collapse = " + ")))
forest.nn2 = pcaNNet(f, forest.train[,idxs], size = 5, linout = FALSE, trace = FALSE, thresh = 0.9)
forest.model.nn2 = sample
