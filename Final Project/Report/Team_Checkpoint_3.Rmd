...
library(neuralnet)
set.seed(444)
forest.model.nn5 = sample_n(forest.train4, 40000)
n = names(forest.model.nn5)
ft = as.formula(paste("~", paste(n, collapse = " + ")))
m = model.matrix( ft , data=forest.model.nn5)[,-1]
m = cbind(m, forest.model.nn5$CoverType)
colnames(m)[ncol(m)] = "CoverType"
n = colnames(m)
fnn = as.formula(paste("CoverType ~", paste(n[!n %in% "CoverType"], collapse = " + ")))
forest.nn5 = neuralnet(fnn, data=m, hidden=5, linear.output=FALSE, learningrate = 0.01)

n = names(forest.valid2)
fv = as.formula(paste("CoverType ~", paste(n[!n %in% "CoverType"], collapse = " + ")))
m.test = model.matrix( fv , data=forest.valid2)[,-1]
m.test = cbind(m.test, forest.valid2$CoverType)
