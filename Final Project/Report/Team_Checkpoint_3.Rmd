...
forest.train4 = forest.model4[indices.train,]
...
An analysis of comparing the kappa values on a random forest model of these samples shows that a random sampling of 40,000 performs the best. Though common sense will tell us to include all observations of the less popular cover types. Therefore the sampling used in this modeling will include a tiered selection of observations with approximately 40,000 observations in all.
...
For modeling with a neural network, there are several variations that can impact the effectiveness of the model. These include the learning rate, the number of layers in the neural network, and the number of nodes in each layer. Several models were investigated changing these parameters. Additionally, we attempted to use different variables in the modeling. The best perform of the neural networks was: 
...
forest.nn1.pred = predict(forest.nn1, newdata=forest.valid1[,-idx], type="class")
...
#model with soil info and assured low class observations, 7 nodes, 0.01 decay
set.seed(444)
if (build) {
  forest.nn4 = nnet(CoverType ~ . , data=forest.train4, linout=FALSE, size=5, maxit=2000, decay=0.01, trace=FALSE)
  saveRDS(forest.nn4, "./forest.nn4")
} else {
  forest.nn4 = readRDS("./forest.nn4")
}
idx = grep("CoverType", colnames(forest.valid4))
forest.nn4.pred = predict(forest.nn4, newdata=forest.valid4[,-idx], type="class")
...
library(neuralnet)
set.seed(444)
forest.nn5.model = forest.train4
idxs = grep("forest.zone", colnames(forest.nn5.model))
m = dummyVars(" ~ .", data = forest.train4[,-idxs])
m = predict(m, newdata = forest.train4)
...
if (build) {
  forest.nn5 = neuralnet(fnn, data=m, hidden=c(5,3), linear.output=FALSE, learningrate = 0.01)
  saveRDS(forest.nn2, "./forest.nn5")
} else {
  forest.nn5 = readRDS("./forest.nn5")
}

m.test = dummyVars(" ~ .", data = forest.valid4)
m.test = predict(m, newdata = forest.valid4)
m.test$forest.area.Area1 = NULL
m.test$forest.zone.trans.zone27 = NULL
forest.nn5.pred = compute(forest.nn5, m.test[,1:ncol(m.test)-1])$net.result
...
forest.valid.x <- forest.valid1[,-c(4,9,10)]
forest.valid.y <- forest.valid1[,9]
...
colnames(forest.train.knn3)[8] <- "CoverType"
colnames(forest.train.knn3)[9] <- "forest.area"
colnames(forest.train.knn3)[10] <- "forest.zone"
...
forest.valid.knn3 <- cbind(df.std.valid,
                       forest.valid1$CoverType, 
                       forest.valid1$forest.area,
                       forest.valid1$forest.zone)
colnames(forest.valid.knn3)[8] <- "CoverType"
colnames(forest.valid.knn3)[9] <- "forest.area"
colnames(forest.valid.knn3)[10] <- "forest.zone"
...
#Modeling
pred <-knncat(forest.train.knn3, 
              forest.valid.knn3,
              classcol = 9,
              k= c(2,3,4,5,6,7,8,9,10), #values of K to test
              xvals = 5) #cross-validation folds
pred$best.k #K=3 from CV
pred$misclass.mat #Confusion Matrix
pred #44.1% misclassiciation rate
