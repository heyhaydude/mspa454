...
We will use âKappaâ as a metric to optimize for tuning our parameters because this metric can improve the quality of the model for problems where there are a low percentage of samples in one class.
...
K Nearest Neighbor (KNN) modeling involves a relatively simple algorithm that classifiers a target variable based on the classification of the âkâ nearest neighbors. K is selected using cross-validation to avoid over-fitting. For our model, we used ten-fold cross validation across K values ranging from 2-12. Predictor variables were normalized across a random sample of 40,000 training observations. The best performing model used three Shade PCA variables along with summarized Soil Types in the form of Forest Zones. An optimal âkâ value of 9 was elected for this model, which resulted in model that had a 19.1% misclassification rate and Kappa value of 0.6983.
...
When working with a multiple classification problem, it is important to evaluate a model based on more than just its accuracy. Although accuracy is important, it can sometimes under-represent classes. To give a holistic assessment of a model, we also look at the model's kappa value. The kappa metric, which is a measured comparison of both observed accuracy and expected accuracy, penalizes a model only predicting some of the classes well, and leaving out others. Balanced accuracy measures accuracy in each of the classes, and gives a more detailed look at the accuracy of the model.

Our highest performing model was the random forest model with Principal Components Analysis. This model had an accuracy of 90.8%. Other advantages of the model were that it had a kappa value of 85.1%, which is very high in terms of representing all forest cover types. Looking at the Balanced Accuracy plot of this model below, we see that the accuracy values for each class are all above 75%.
...
Our preliminary exploratory analysis revealed some potentially useful predictors and helped us to understand the relationships in our data. Principal components analysis helped us narrow our soil variables down from 40 to 16 variables. Additionally, we prepared the data by taking a random sample of the data to run models on, creating a subset of 70% of the data to train our models on, and a validation set of 30% of the data to test those models on. 

Next we experimented with different samples, feature sets and several classification algorithms and assessed our initial modeling results. We used these results to develop neural network, random forest, logistic regression, linear discriminant analysis, k nearest neighbor, and support vector machine models. We evaluated and compared these six model types based on their accuracy, kappa values and balanced accuracy. In the end, our best performing model for predicting forest cover type, was ironically, random forest (wink wink). 
